...
// Enhanced CORS configuration to allow frontend on port 5173 and 5174 (Vite default)
app.use(cors({
...
// Get impulsive purchases endpoint
app.get("/api/impulsive", (req, res) => {
  try {
    const impulsiveData = JSON.parse(readFileSync('./files/impulsive.json', 'utf-8'));
    res.json(impulsiveData);
  } catch (error) {
    console.error('Error loading impulsive purchases:', error);
    res.status(500).json({ 
      error: 'Failed to load impulsive purchases',
      message: error.message 
    });
  }
});
