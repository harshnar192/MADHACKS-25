...
function SummaryPage() {
  ...
  const handleGenerateAISummary = async () => {
    ...
    try {
      ...
      const data = {
        ...
        voiceEntries: [],
        invisibleSpending: {
          unlogged_transactions: impulsivePurchases.filter(tx => (tx.amount || 0) < 50).length,
          unlogged_amount: impulsivePurchases.filter(tx => (tx.amount || 0) < 50).reduce((sum, tx) => sum + (tx.amount || 0), 0)
        },
        patterns: emotionalTriggers.reduce((acc, trigger, idx) => {
          acc[`pattern_${idx}`] = {
            pattern: trigger.trigger,
            occurrences: trigger.count,
            total_cost: (emotionalSpending || 0) * (trigger.count / impulsivePurchases.length) || 50 * trigger.count
          };
          return acc;
        }, {})
      };
  ...
  const weeklyInsight = impulsivePurchases.length > 0
  ...
  // Generate coaching text based on actual data from impulsive purchases
  const coachingText = impulsivePurchases.length > 0
    ? `You've made ${impulsivePurchases.length} impulse purchases with a total of $${(emotionalSpending || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} in emotional spending. Your primary triggers are: ${emotionalTriggers.slice(0, 3).map(t => `${t.trigger} (${t.count})`).join(', ')}. Consider setting up reminders to pause before making purchases when you notice these patterns.`
    : checkIns.length > 0
      ...
      <Row className="g-3 mb-4">
        ...
        <Col md={6}>
          <InsightCard 
            ...
            subtitle={loadingImpulsive ? "Loading..." : `Total: $${(emotionalSpending || totalSpent || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
            accent="primary"
          >
